...
@include('sweetalert::alert')
	<div class="max-w-[90rem] mx-auto p-6">
		<h1 class="text-3xl dark:text-gray-300 text-center font-semibold m-4">Black Sidewall 2 Compound</h1>
		<form action="{{ route('chart.chart-black-sidewall-2-compd') }}" method="POST" class="flex">
			@csrf
				...
				<div class="flex gap-3 items-center">
					...
					<div class="flex gap-2 overflow-x-auto mb-5">
						@for ($i=1; $i<= 20; $i++)
							...
							<input type="number" name="{{ 'gbec'.$i }}" class="{{ $i === 1 ? 'ms-1' : '' }} w-10 bg-slate-100 px-2 py-1 mt-1 border rounded text-center focus:ring focus:ring-blue-300" step="any" value="{{ old('gbec'.$i) }}">
						@endfor
					</div>
				</div>
				<div class="mt-7">
